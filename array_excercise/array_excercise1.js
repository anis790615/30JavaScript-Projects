...
//or links =  [...document.querySelectorAll('.mw-category a')];
const streetsWithDe = links
  .map(link => link.title)
  .filter(street => street.includes("de"));
console.log(streetsWithDe); */
...
const countTransport = data.reduce((obj, item) => {
  if (!obj[item]) {
    obj[item] = 0;
  }
  obj[item]++;
